...
if __name__ == '__main__':
    ...
    for student in list_of_students:
        if not os.path.exists(f'./CW_rugby/rugby_{student}'):
            os.mkdir(f'./CW_rugby/rugby_{student}')

            #Make tesfiles folder
            make_input = ['mkdir', f'./CW_rugby/rugby_{student}/test_files_rugby', ]
            subprocess.call(make_input)
            #Make results folder

            #copy test files into test files folder
            copy_input = ['cp', '-r', f'./test_files_rugby', f'./CW_rugby/rugby_{student}', ]
            subprocess.call(copy_input)
            #Create feedback file

        #Delete and remake results files
        if os.path.exists(f'./CW_rugby/rugby_{student}/results_rugby'):
            df = ['rm', '-r', f'./CW_rugby/rugby_{student}/results_rugby', ]
            subprocess.call(df)
        make_input2 = ['mkdir', f'./CW_rugby/rugby_{student}/results_rugby', ]
        subprocess.call(make_input2)

        #Delete and remake feedback files on new run

        if os.path.exists(f'./CW_rugby/rugby_{student}/fback.txt'):
            feedbacks = ['rm', f'./CW_rugby/rugby_{student}/fback.txt', ]
            subprocess.call(feedbacks)
            feedback = ['touch', f'./CW_rugby/rugby_{student}/fback.txt', ]
    ...
    for student in list_of_students:
        ...
        for file in py_files:
            if file.endswith('.py'):
                ...
                if file == (f'rugby_{student}.py'):
                    correct_program_name = True




                    inputs = ['python3', f'./rugby_{student}.py', './test_files_rugby', './results_rugby']
                    ...
                    try:
                        ...
                        errors = errors.replace("/home/jake/Desktop/repos/Marking/CW_rugby", "***Removed due to privacy***")
                        print(errors)
        ...
        with open(f'./fback.txt', 'a') as f:
            if file_exists == False:
                f.write("No submission was found under the prog1_rugby tag for the rugby task\n")
            else:
                if correct_program_name == False:
                    f.write("Submission was found under the prog1_rugby tag for the rugby task but the file is named incorrectly\n")
